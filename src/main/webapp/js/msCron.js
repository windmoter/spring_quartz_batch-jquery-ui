...
(function ($) {
    ...
    $.fn.msCron = function (options) {
        ...
        var topMenu = $("<ul/>", {"class": "nav nav-tabs", id: "msCronTabs", style: "font-size:13px"});
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link active" id="msSecondsTab" role="tab" data-toggle="tab" href="#Seconds">Seconds</a>')).appendTo(topMenu);
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link" id="msMinutesTab" role="tab" data-toggle="tab" href="#Minutes">Minutes</a>')).appendTo(topMenu);
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link" id="msHourlyTab" role="tab" data-toggle="tab" href="#Hourly">Hourly</a>')).appendTo(topMenu);
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link" id="msDailyTab" role="tab" data-toggle="tab" href="#Daily">Daily</a>')).appendTo(topMenu);
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link" id="msWeeklyTab" role="tab" data-toggle="tab" href="#Weekly">Weekly</a>')).appendTo(topMenu);
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link" id="msMonthlyTab" role="tab" data-toggle="tab" href="#Monthly">Monthly</a>')).appendTo(topMenu);
        $('<li/>', {'class': 'nav-item'}).html($('<a class="nav-link" id="msYearlyTab" role="tab" data-toggle="tab" href="#Yearly">Yearly</a>')).appendTo(topMenu);
        $(topMenu).appendTo(mainDiv);
        var tabContent = $("<div/>", {"class": "tab-content"});

        //creating the secondsTab
        var secondsTab = $("<div/>", {"class": "tab-pane active", id: "Seconds"});
        var secondsTabSection = $("<div/>", {"style": "padding:15px"});
        $(secondsTabSection).append("Every&nbsp;");
        $("<input/>", {id: "SecondsInput", type: "text", value: "30", class: "form-control", style: "width: 50px;display:inline"}).appendTo(secondsTabSection);
        $(secondsTabSection).append("&nbsp;second(s)");
        $(secondsTabSection).appendTo(secondsTab);
        ...
        $(hourlyOption1).append(' starting at hour <select id="everyAtHours" class="form-control hours" style="width: 80px;display:inline;margin-right:5px;"></select>');
        ...
        $(hourlyOption2).append("&nbsp;minute(s) every hour&nbsp;");
        ...
        function generate() {
            ...
            var activeTab = $("ul#msCronTabs li a.active").prop("id");
                ...
                case "msHourlyTab":
                    switch ($("input:radio[name=HourlyRadio]:checked").val()) {
                        case "1":
                            ...
                            results = "0 0 " + Number($("#everyAtHours").val())+ "/" + $("#HoursInput").val() + " 1/1 * ? *";
                            break;
                        case "2":
                            results = "0 " + Number($("#AtMinutes").val()) + " * 1/1 * ? *";
                            break;
                    }
                    activeTab = "HOURLY";
                    break;
        ...
        function populateCron(actTab, expression) {
            ...
            if (validExp.length === 7) {
                switch (actTab) {
                    case "SECONDS":
                        $('#msCronTabs a[href="#Seconds"]').tab('show');
                        expression = expression.match(/\d+/g);
                        $("#SecondsInput").val(expression[2]);
                        break
                    case "MINUTES":
                    ...
                    case "HOURLY":
                        ...
                        expression = expression.match(/\d+/g);
                        // Modification logic needed to changed for ("2/1" format)
